...
src_install() {
	...
	do
		...
		fowners root:video "${install_dir}/${runtime_dir}"
		fperms g+ws "${install_dir}/${runtime_dir}"
	done

	# Resolve creates additional runtime directories under ${install_dir}
	# that are NOT knowable in advance -- confirmed via strace hitting
	# mkdir("${install_dir}/Immersive", 0777) = EACCES on first launch,
	# a top-level path this ebuild never declared via keepdir. A new
	# directory entry requires write+execute on its PARENT, so no matter
	# how many specific runtime_dir paths above get fixed, any directory
	# Resolve invents at runtime that isn't already listed will fail the
	# same way. Rather than chasing these one strace at a time, make the
	# install root itself group-writable + setgid so new entries Resolve
	# creates there inherit group ownership and stay writable. This does
	# NOT touch permissions on bin/, libs/, plugins/, scripts/, or share/
	# -- those keep the strict root:root 0755 already set above; only
	# the top-level directory and the already-declared runtime dirs
	# above gain group write access.
	fowners root:video "${install_dir}"
	fperms g+ws "${install_dir}"
...
pkg_postinst() {
	...
	elog
	elog "Resolve creates several directories under ${EPREFIX}/opt/${PN} at"
	elog "runtime that are not known in advance. These are group-writable to"
	elog "the 'video' group -- if the user running Resolve is not already a"
	elog "member, add them and re-login:"
	elog
